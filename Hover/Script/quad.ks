...
//if not(moduleName = false) { //undock dockingport tagged moduleName
//	set taggedParts to ship:partstagged(moduleName).
//	for p in taggedparts {
//		for m in p:modules {
//			if m:hasevent("Decouple") m:doevent("Decouple Node").
...
for eng in engs {
	...
	set rot to 0.
	local parentModules is eng:parent:modules.
	for moduleStr in parentModules {
		if moduleStr = "MuMechToggle" {
			set rot to eng:parent:getmodule("MuMechToggle").
			rot:setfield("Acceleration",50).
			print "Found engine yaw rotatron.".
...
if cam:length > 0 and camRotH:length > 0 and camRotV:length > 0 {
	print "Found camera and gimballing parts, enabling camera mode..".
