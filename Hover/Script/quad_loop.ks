function flightcontroller {
	...
	if yawControl {
		...
		print "yawRot: " + yawRotatrons[0]:getfield("Rotation") at (0,terminal:height-2).
		print "yawAngVel: " + round(yawAngVel,2) + "  " at (0,terminal:height-1).
		
		if ship:control:pilotroll > 0 set targetRot to 6.
		else if ship:control:pilotroll < 0 set targetRot to -6.
		else {
			if abs(yawAngVel) > 1 set targetRot to min(20,abs(yawAngVel)^2 * 5).
			else set targetRot to min(20,sqrt(abs(yawAngVel)) * 5).
			if yawAngVel > 0 set targetRot to -targetRot.
		}
		
		for rotMod in yawRotatrons {
			local curRot is rotMod:getfield("Rotation").
			set yawAngErr to targetRot - curRot.
			rotMod:setfield("speed",max(0,min(3,abs(yawAngErr)/10))).
			if yawAngErr < -0.01 rotMod:doaction("move -",true).
			else if yawAngErr > 0.01 rotMod:doaction("move +",true).
			else { rotMod:doaction("move +",false). rotMod:doaction("move -",false). }
		}
