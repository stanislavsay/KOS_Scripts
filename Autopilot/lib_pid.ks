...
function PID_init {
  parameter
    ...
    deadband. // value of P which is deadband if below
  ...
  //
  local PID_array is list(Kp, Ki, Kd, cMin, cMax, SeekP, P, I, D, oldT, oldInput, deadband).
...
function PID_seek {
  ...
  local deadband is PID_array[11].
  ...
  if oldT < 0 {
    ...
    if dT > 0 and abs(P) >= deadband { // Do nothing if no physics tick has passed from prev call to now.
...
function setMinMax {
  parameter
    PID_array, // array built with PID_init.
    minVal,   // value we want.
    maxVal.    // value we currently have.

    set PID_array[3] to minVal.
    set PID_array[4] to maxVal.
}
